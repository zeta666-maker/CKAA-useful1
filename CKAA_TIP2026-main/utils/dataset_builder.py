...
from utils.tabular_data import TabularPathDataset, define_tabular_dataset
...
class CIFAR100Path(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        ...
        for cls_dir in cls_dir_list: ## len(cls_dir_list) = 100
            cls_int = int(cls_dir)
...
class ImageNetAPath(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        super().__init__()
        cls_dir_list = sorted(os.listdir(split_path := join(root_dir, 'train' if train else 'val')))
        assert exists(split_path), split_path
        ...
        name_map = {}
        if exists(_rf := join(f"{dirname(dirname(realpath(__file__)))}/tools/imagenet_a_classnames.txt")):
            with open(_rf, 'r') as f:
                for line in f.readlines():
                    _sn = line.split('\t')[0]
                    _dn = line.strip('\n')[len(_sn) + 1:]
...
class StanfordCarsPath(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        super().__init__()
        cls_dir_list = sorted(os.listdir(split_path := join(root_dir, 'train' if train else 'val')))
        assert exists(split_path), split_path
        ...
        assert len(self.class_imgpath_dict) == 196
...
class ImagePathDatasetClassManager():
    def __init__(self, **kwargs):
        self.dataset_dict = {
            ...
            'imagenet_a': partial(ImageNetAPath, root_dir="../datasets/data.ImageNet-A" if not (v := kwargs.get('imagenet_a')) else v),
...
class ClassIncremantalDataset(Dataset):
    def __init__(self, path_dataset: ImagePathDataset, 
                task_class_list: list[int], transforms: T.Compose = None, 
                target_transforms: Callable = None, expand_times: int = 1, 
                return_index: bool = True, sample_type='path'):
        ...
        self.samples, self.labels = self.get_all_samples(sample_type=self.sample_type) ## image:PIL; transformed labels:int
        self.return_index = return_index
        self.num_samples = len(self.labels) ## 5000 = 10 * 500
...
def define_dataset(GVM, task_classes: list[int], training: bool, transform_type: str = 'timm', target_map_to_local: bool = True,
    ...
    match _current_dataset:
        case 'imagenet_r' | 'sdomainet' | 'imagenet_a' | 'cub' | 'stanford_cars':
    ...
    match transform_type:
        ...
        case 'clip':
            ...
            if training and not use_eval_transform:
                ...
                match _current_dataset:
                    case 'cifar100' | 'imagenet_r' | 'sdomainet' | 'imagenet_a' | 'cub' | 'stanford_cars':
    ...
    target_transforms = TargetTransform(GVM.label_map_g2l, target_map_to_local) ## label2idx: {55:0; 17:1, ...}

    _mode = 'train' if training else 'eval'
    dataset = ClassIncremantalDataset(GVM.path_data_dict[_mode], task_classes, transforms, 
                                    target_transforms, expand_times=expand_times, return_index=True, 
                                    sample_type=GVM.args.sample_type)
