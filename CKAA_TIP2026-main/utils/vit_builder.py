...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if 'pid1' in pid_dict and 'pid2' in pid_dict:
            self.interm_reader_1 = IntermReader(pid_dict['pid1'], 'interm_reader_1', {'w_qkv': self.qkv.weight})
            self.interm_reader_2 = IntermReader(pid_dict['pid2'], 'interm_reader_2')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if hasattr(self, (_reader := 'interm_reader_1')): ## if self has 'interm_reader_1' module
    ...
    def forward_moe(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = self.qkv(x).reshape(Ne, B, N, 3, self.num_heads, self.head_dim).permute(0, 3, 1, 4, 2, 5)
        q, k, v = qkv.unbind(1)  # [B, H, N, d] * 3
        q, k = self.q_norm(q), self.k_norm(k)
        ...
        x = x.transpose(2, 3).reshape(Ne, B, N, C)        
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class TaskSharedPrompt(nn.Module):
    ...
    def forward(self, x: Tensor, stop_grad: bool=False, forward_prompt: bool=False):
        batch_prompt = self.prompt.expand(x.shape[0], -1, -1) ## 1,4,768 --> B,4,768
        if stop_grad:
        ...
        if self.replace_token:
            ...
            out = torch.concat([x, batch_prompt], dim=1) ## cat [x:B, 197, 768, prompts:B, 4, 768]
...
class TaskSpecificPrompt(nn.Module):
    def __init__(self, prompt_len: int, num_tasks: int, embed_dim: int = 768, prompt_init: str = 'uniform'):
        super().__init__()
        assert prompt_len >= 0
        self.prompt_len = prompt_len
        self.embed_dim = embed_dim
        ...
        if prompt_init == 'zero':
            self.prompt = nn.ParameterList([nn.Parameter(torch.zeros(1, prompt_len, embed_dim)) 
                        for i in range(num_tasks)])
        elif prompt_init in ('uniform', ''):
            self.prompt = nn.ParameterList([nn.Parameter(torch.randn(1, prompt_len, embed_dim)) 
                        for i in range(num_tasks)])
    ...
    def forward(self, x: Tensor, taskid: int=None, max_taskid=None, forward_prompt: bool=False):

        batch_prompt = None
        if taskid is not None and isinstance(taskid, int):
            batch_prompt = self.prompt[taskid].expand(x.shape[0], -1, -1)
        elif taskid is not None and isinstance(taskid, torch.Tensor):
            assert max_taskid is not None
            prompt_pool = torch.cat([self.prompt[i] for i in range(max_taskid+1)], dim=0)
            batch_prompt = prompt_pool[taskid]
        ...
        if forward_prompt:
            if batch_prompt is not None:
                ...
                assert max_taskid is not None
                batch_prompt = torch.cat([self.prompt[i] for i in range(max_taskid+1)], dim=0)
                return batch_prompt

        out = torch.concat([x, batch_prompt], dim=1)
        assert out.shape[1] == x.shape[1] + self.prompt_len
        return out
    
    def extra_repr(self) -> str:
        return f"prompt_len={self.prompt_len}, num_tasks={self.num_tasks}, embed_dim={self.embed_dim}, prompt_init={self.prompt_init}"
    



class TaskPrompt(nn.Module):
    def __init__(self, prompt_len: int, embed_dim: int = 768, prompt_init: str = 'uniform'):
        super().__init__()
        assert prompt_len > 0
        self.prompt_len = prompt_len
        self.embed_dim = embed_dim
    ...
    def extra_repr(self) -> str:
        return f"prompt_len={self.prompt_len}, embed_dim={self.embed_dim}, prompt_init={self.prompt_init}"



class Adapter(nn.Module):

    def __init__(self, embed_dim: int = 768, dropout: float = 0.0, init_option:str = "lora",
                adapter_layernorm_option: str = 'in', adapter_scalar: str = '1.0'):
        super().__init__()
        ...
        self.middle_dim = 64
        self.down_proj = nn.Linear(self.embed_dim, self.middle_dim)
        self.non_linear_func = nn.ReLU()
        self.up_proj = nn.Linear(self.middle_dim, self.embed_dim)
...
class TaskNet(nn.Module):
    def __init__(self, embed_dim: int=768, middle_dim: int=64):
        super().__init__()

        self.embed_dim = embed_dim
        self.relu = nn.ReLU()
        self.down_proj = nn.Linear(embed_dim, middle_dim)
        self.up_proj = nn.Linear(middle_dim, embed_dim)
    ...
    def forward(self, x):
        ...
        x = self.up_proj(self.relu(self.down_proj(x)))
...
class Block(nn.Module):
    def __init__(
            ...
            sh_prompt: TaskSharedPrompt = None,
            sp_prompt: TaskSpecificPrompt = None, 
            t_prompt: TaskPrompt = None,
    ) -> None:
        super().__init__()

        if sh_prompt is not None:
            assert isinstance(sh_prompt, TaskSharedPrompt), type(sh_prompt)
            self.sh_prompt = sh_prompt
            pid_dict = {'pid1': id(self.sh_prompt.prompt),
                        'pid2': id(self.sh_prompt.prompt)}
        else:
            pid_dict = {}

        if sp_prompt is not None:
            assert isinstance(sp_prompt, TaskSpecificPrompt), type(sp_prompt)
            self.sp_prompt = sp_prompt

        if t_prompt is not None:
            assert isinstance(t_prompt, TaskPrompt), type(t_prompt)
        ...
        self.num_tasks = self.sp_prompt.num_tasks
        self.stop_grad_for_sh = False
        self.dim = dim
        self.num_patch = 197
        self.prompt_linear = nn.Linear(self.num_patch-1, self.sp_prompt.prompt_len, bias=False) 

        self.task_adapter = nn.ModuleList()
        for i in range(self.num_tasks):
            adapter = Adapter(embed_dim=self.dim, dropout=0.1,
                            init_option='lora',
                            adapter_layernorm_option='none',
                            adapter_scalar=0.1,
                            )
            self.task_adapter.append(adapter)

    def forward(self, x: torch.Tensor, mode: str=None, taskid: int=None, 
            max_taskid: int=None, p_task: torch.Tensor=None, eval_adapter_scale: float=None) -> torch.Tensor:
        
        if mode is not None:
            if mode == 'shared':
                if hasattr(self, 'sh_prompt'):
                    _fn1, _fn2 = self.sh_prompt, self.norm1
                    x_a = self.attn(_fn2(_fn1(x)))[:, :self.sh_prompt.pre_tokens]

            elif mode == 'specific':
                if hasattr(self, 'sp_prompt'):
                    assert taskid is not None
                    _fn1, _fn2 = self.sp_prompt, self.norm1
                    x_a = self.attn(_fn2(_fn1(x, taskid)))[:, :self.sp_prompt.pre_tokens]

            elif 'prompt' in mode:
                
                if hasattr(self, 'sh_prompt'):
                    sh_prompt = self.sh_prompt(x, stop_grad=self.stop_grad_for_sh, forward_prompt=True)
                if hasattr(self, 'sp_prompt'):
                    assert taskid is not None
                    if p_task is None:
                        sp_prompt = self.sp_prompt(x, taskid=taskid, max_taskid=max_taskid, forward_prompt=True)
                    else:
                        batch_size = x.shape[0]
                        sp_prompt = self.sp_prompt(x, taskid=None, max_taskid=max_taskid, forward_prompt=True)
                        sp_prompt = p_task.mm(sp_prompt.reshape(max_taskid+1, -1))
                        sp_prompt = sp_prompt.reshape(batch_size, self.sp_prompt.prompt_len, -1)
                
                if mode == 'prompt_train':
                    batch_prompt = sp_prompt
                elif mode == 'prompt_eval':
                    batch_prompt = (sh_prompt + sp_prompt) / 2

                x_a = torch.cat([x, batch_prompt], dim=1)
                x_a = self.attn(self.norm1(x_a))[:, :self.sh_prompt.pre_tokens]
                ...
                if hasattr(self, 'sh_prompt'):
                    _fn1, _fn2 = self.sh_prompt, self.norm1
                    x_a = self.attn(_fn2(_fn1(x, stop_grad=self.stop_grad_for_sh)))[:, :self.sh_prompt.pre_tokens]

            elif 'selection' in mode: 
                x_a = self.attn(self.norm1(x))
        else:
            x_a = self.attn(self.norm1(x))


        x = x + self.drop_path1(self.ls1(x_a))  ## Identity
        if mode is not None:
            if 'adapter' in mode:
                assert max_taskid is not None
                if 'train' in mode:
                    adapter_out = self.task_adapter[taskid](x, add_residual=False)
                elif 'eval' in mode:
                    adapter_out = torch.cat([self.task_adapter[i](x, add_residual=False).unsqueeze(1) 
                                for i in range(max_taskid+1)], dim=1)
                    if p_task is None:
                        p_task = F.one_hot(taskid, max_taskid+1)
                    
                    adapter_out = (adapter_out * p_task.unsqueeze(2).unsqueeze(3)).sum(1)

                assert x.dim() == 3 and x.shape[-1] == self.dim
                x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x)))) + adapter_out
            else:
                x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x)))) 
        else:
            x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x)))) 
        ## layernorm:768 --> MLP (Linear, GELU, Linear)
...
class MetaNet(nn.Module):
    def __init__(self, embed_dim, num_tasks):
        ...
        norm_layer = get_norm_layer(None) or partial(nn.LayerNorm, eps=1e-6)
        self.norm = norm_layer(self.embed_dim)

    def forward(self, x):

        residual = self.meta_net(x)
...
class CosineClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = x.mm(weight.t()) / self.temperature
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_pos: torch.Tensor, x_neg: torch.Tensor, 
                lbl: torch.Tensor, pos_lbl: torch.Tensor, neg_lbl: torch.Tensor,
                mask: bool=False):

        assert x.shape[0] == lbl.shape[0]
        current_device = x.device
        x_pos, pos_lbl = x_pos.to(current_device), pos_lbl.to(current_device)
        x_neg, neg_lbl = x_neg.to(current_device), neg_lbl.to(current_device)
        batch_size, pos_size, neg_size = lbl.shape[0], pos_lbl.shape[0], neg_lbl.shape[0]
        pos = torch.as_tensor(lbl.expand(pos_size, batch_size).t().eq(pos_lbl.expand(batch_size, pos_size)),\
                        dtype=torch.float) ## [batch_size, t_size]
        neg = torch.as_tensor(lbl.expand(neg_size, batch_size).t().ne(neg_lbl.expand(batch_size, neg_size)),\
                                dtype=torch.float) ## [batch_size, t_size] 
        if mask == True:
            batch_size = x.shape[0]
            pos[torch.arange(batch_size).to(current_device), torch.arange(batch_size).to(current_device)] = 0.
        
        x, x_pos, x_neg = F.normalize(x, dim=1), F.normalize(x_pos, dim=1), F.normalize(x_neg, dim=1)
        numerator = (torch.exp(x.mm(x_pos.t()) / self.temperature) * pos).sum(1)
        denominator = ((torch.exp(x.mm(x_neg.t()) / self.temperature)) * neg).sum(1)
        train_idx = torch.where(numerator > 0.0)[0]

        loss = -(numerator[train_idx] / (numerator + denominator)[train_idx]).log().mean()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                mlp_layer=mlp_layer,
                sh_prompt=TaskSharedPrompt(_pd['prompt_len_sh'], 
                            embed_dim, _pd['prompt_init'], False) 
                            if _pd['prompt_start_block'] <= i <= _pd['prompt_end_block'] 
                            and _pd['prompt_len'] > 0 else None,
                sp_prompt=TaskSpecificPrompt(_pd['prompt_len_sp'],
                            _pd['prompt_num_tasks'],
                            embed_dim, _pd['prompt_init']) 
                            if _pd['prompt_start_block'] <= i <= _pd['prompt_end_block'] 
                            and _pd['prompt_len'] > 0 else None,
                t_prompt = None
            )
        ...
        self.num_tasks = _pd['prompt_num_tasks']

        self.num_clusters_per_class = 3
        self.learnable_prototypes = nn.ParameterList([nn.Parameter(torch.randn(num_classes, 
                                    self.num_clusters_per_class, 
                                    self.embed_dim)) 
                                    for i in range(self.num_tasks)])
        for p in self.learnable_prototypes:
            nn.init.uniform_(p, -1, 1)

        self.task_head = nn.ModuleList([nn.Linear(self.embed_dim, num_classes * self.num_tasks) 
                                    for i in range(self.num_tasks)])
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        to_cat = []
        if self.cls_token is not None:
            to_cat.append(self.cls_token.expand(x.shape[0], -1, -1)) ## B, 1, 768
        ...
        if self.no_embed_class:
            ...
            if to_cat:
                x = torch.cat(to_cat + [x], dim=1) 
            x = x + pos_embed ## x: B * 197 * 768; pos_embed:256 * 197 * 768
    ...
    def forward_features(self, x: torch.Tensor, mode: str=None, taskid: int=None, 
                        max_taskid: int=None, p_task: torch.Tensor=None, eval_adapter_scale:float=None) -> torch.Tensor:
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            ## 12 blocks, each block contains 4 shared prompts: self.blocks[i].prompt.prompt, i=0,1,...11
            if mode is not None:
                if any(_s in mode for _s in ['shared', 'selection']):
                    # x = self.blocks(x, mode)
                    for blk in self.blocks:
                        x = blk(x, mode)
                elif any(_s in mode for _s in ['specific', 
                                'prompt_train', 'prompt_eval', 
                                'adapter_train', 'adapter_eval']):
                    assert taskid is not None
                    for blk in self.blocks:
                        x = blk(x, mode, taskid, max_taskid, p_task, eval_adapter_scale)
            else:
                for blk in self.blocks:
                    x = blk(x, mode=None)
    ...
    def forward_head(self, x: torch.Tensor, pre_logits: bool = False) -> torch.Tensor:
        ...
        if pre_logits:
            ...
            if self.previous_head is not None:
                return [x, self.head(x), self.previous_head(x)]
            else:
                return [x, self.head(x), None]
    ...
    def encode_image(self, x, mode: str=None, taskid: int=None, max_taskid: int=None, 
                    p_task: torch.Tensor=None, eval_adapter_scale: float=None, pre_logits: bool = False):
        x = self.forward_features(x, mode, taskid, max_taskid, p_task, eval_adapter_scale) 
        ## [conv,cls_token,pos_embed] --> 12 blocks --> layernorm
        if mode == 'selection_training':
            pre_logits: bool = True
            x, x_pre = self.forward_head(x, pre_logits)
            keys = torch.cat([self.keys[i] for i in range(taskid+1)], dim=0)
            x, keys = F.normalize(x, dim=1), F.normalize(keys, dim=1)
            task_logits = x.mm(keys.permute(1, 0))
            return task_logits
        elif mode == 'selection_evaluation':
            pre_logits: bool = True
            x, x_pre = self.forward_head(x, pre_logits)
            keys = torch.cat([self.keys[i] for i in range(taskid+1)], dim=0)
            x, keys = F.normalize(x, dim=1), F.normalize(keys, dim=1)
            ...
            feat, x, x_pre = self.forward_head(x, pre_logits) ## classifier: 768 --> 10
            return feat, x, x_pre
    ...
    def forward(self, x: torch.Tensor, mode: str=None, taskid: int=None, 
                max_taskid: int=None, p_task: torch.Tensor=None, eval_adapter_scale: float=None, 
                pre_logits: bool=False) -> torch.Tensor:
        # if mode in ['selection', 'evaluation']:
        #     _, score = self.forward_features(x, mode, taskid)
        #     return score
        # else:
        if 'selection' in mode:
            task_logits = self.encode_image(x, mode, taskid, pre_logits) ## classifier logit: B * 10
            return task_logits
        else:
            feat, x, x_pre = self.encode_image(x, mode, taskid, max_taskid, 
                        p_task, eval_adapter_scale, pre_logits) ## classifier logit: B * 10
...
class TabularPatchEmbed(nn.Module):
    """1D tokenizer for interleaved [vibration, pressure-XYZ] windows."""
    ...
    def __init__(
        ...
        super().__init__()
        if signal_length <= 0 or patch_size <= 0 or in_chans <= 0:
            raise ValueError("signal_length, patch_size and in_chans must be positive")
        flattened_length = signal_length * in_chans
        if flattened_length % patch_size:
            raise ValueError(
                f"{signal_length} * {in_chans} must be divisible by patch_size={patch_size}"
            )
        self.signal_length = signal_length
        self.patch_size = patch_size
        self.in_chans = in_chans
        self.embed_dim = embed_dim
        self.num_patches = flattened_length // patch_size
    ...
    def forward(self, x: Tensor) -> Tensor:
        if x.ndim != 3:
            raise ValueError(f"Tabular input must be [B, C, L], got {tuple(x.shape)}")
        batch_size, channels, length = x.shape
        if channels != self.in_chans or length != self.signal_length:
            raise ValueError(
                f"Expected input [B, {self.in_chans}, {self.signal_length}], "
                f"got {tuple(x.shape)}"
            )
        # Time-major interleaving keeps each 32-value patch local across all channels.
        x = x.transpose(1, 2).reshape(batch_size, 1, length * channels)
        x = self.proj(x).transpose(1, 2)
        return self.norm(x)


def convert_vit_to_tabular(
    model: "VisionTransformer",
    signal_length: int = 1568,
    patch_size: int = 32,
    in_chans: int = 4,
) -> "VisionTransformer":
    """Replace a ViT image patch embedding while retaining the pretrained blocks."""
    tabular_embed = TabularPatchEmbed(
        signal_length=signal_length,
        patch_size=patch_size,
        in_chans=in_chans,
        embed_dim=model.embed_dim,
    )
    if model.pos_embed.shape[1] != tabular_embed.num_patches + model.num_prefix_tokens:
        raise ValueError(
            f"Tabular tokenizer creates {tabular_embed.num_patches} patches, but "
            f"the model expects {model.pos_embed.shape[1] - model.num_prefix_tokens}"
        )
    model.patch_embed = tabular_embed
    model.is_tabular = True
    model.tabular_signal_length = signal_length
    model.tabular_patch_size = patch_size
    model.tabular_in_chans = in_chans
    return model
...
def _create_vision_transformer(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    # pretrained_cfg.__dict__.pop('url')
    # pretrained_cfg.__dict__['file'] = 'hlf_CL/A_pretrained_model/ViT-B_16.npz'
