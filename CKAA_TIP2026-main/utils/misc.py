...
def make_label_maps(taskid: int, task_classes: list[int], num_classes_per_task: int) -> dict[int, tuple[int, int]]:
    # g2l_map = {global_label: (taskid, local_label)}
    g2l_map = {}
    for idx, cls in enumerate(task_classes):
        g2l_map[cls] = (taskid, idx, taskid * num_classes_per_task + idx)
    return g2l_map
